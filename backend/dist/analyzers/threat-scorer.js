 ...
 */
export function calculateThreatScore(analysis) {
    const indicators = [];
    ...
    if (analysis.structure.apis && analysis.structure.apis.length > 0) {
        const isExecutableType = ['pe', 'elf', 'macho', 'script', 'html', 'pdf'].includes(analysis.fileIdentification.identifiedType);
        if (!isExecutableType) {
    ...
    const riskLevel = normalizedScore < 20 ? 'safe' :
 ...
 */
function generateScoreExplanation(indicators, riskLevel, score) {
