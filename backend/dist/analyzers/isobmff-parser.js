 ...
 */
export function parseISOBMFF(buffer) {
    try {
        ...
        if (ftypBox)
            confidence += 20;
        if (fileType)
            confidence += 15; // Recognized brand
        if (hasMetaBox || hasMoovBox)
            confidence += 10; // Has metadata
        if (hasMdatBox)
            confidence += 5; // Has media data
        return {
            valid: true,
 ...
 */
export function isLikelyISOBMFF(buffer) {
    if (buffer.length < 12)
        return false;
