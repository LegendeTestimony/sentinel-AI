...
function parseThreatResponse(text) {
    ...
    const extractSection = (sectionName) => {
        ...
        const pattern = new RegExp(`\\*{0,2}${escapedName}\\*{0,2}[:\\s]*\\n?([\\s\\S]*?)(?=\\n\\*{0,2}[A-Z][A-Z ]{2,}\\*{0,2}:|$)`, 'i');
    ...
    const level = threatLevelMatch?.[1]?.toUpperCase() || 'UNKNOWN';
    ...
    if (hiddenContentText) {
        ...
        let contentThreatLevel = 'HARMLESS';
 ...
 */
function getFormatContext(fileType, category) {
    const contexts = {
 ...
 */
export async function analyzeWithGemini(fileAnalysis, fileIdentification, entropyAnalysis, threatScore, steganography) {
    try {
        ...
        const structuredData = {
            ...
            localThreatScore: threatScore ? {
                ...
                benignIndicators: threatScore.indicators.filter((i) => i.weight < 0).map((i) => i.name),
                riskIndicators: threatScore.indicators.filter((i) => i.weight > 0).map((i) => ({
                    name: i.name,
                    weight: i.weight
                }))
            } : null,
