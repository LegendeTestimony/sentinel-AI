...
Be aggressive in your analysis. Assume worst-case scenarios. Your job is to protect users.`;
    ...
    return {
        agent: 'prosecutor',
        analysis: text,
        threatLevel: levelMatch?.[1]?.toUpperCase() || 'MEDIUM',
...
Be thorough in finding innocent explanations. Consider legitimate use cases. Your job is to prevent false positives.`;
    ...
    return {
        agent: 'defense',
        analysis: text,
        threatLevel: levelMatch?.[1]?.toUpperCase() || 'LOW',
 ...
 */
export async function analyzeWithMultiAgent(fileAnalysis, fileIdentification, entropyAnalysis, threatScore, steganography) {
    ...
    const structuredData = {
        ...
        localThreatScore: threatScore ? {
            ...
            riskIndicators: threatScore.indicators.filter((i) => i.weight > 0)
        } : null,
