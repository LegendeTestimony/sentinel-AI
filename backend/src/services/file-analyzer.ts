 ...
 */
export async function analyzeFile(file: Express.Multer.File): Promise<EnhancedAnalysisResult> {
  ...
  const threat = await analyzeWithGemini(
    analysis,
    fileIdentification,
    entropyAnalysis,
    threatScore,
  ...
  const shouldTriggerMultiAgent = (
    threat.level === 'MEDIUM' && threat.confidence < 70
  ) || (
    threat.confidence < 60 && threat.level !== 'SAFE'
  );

  if (shouldTriggerMultiAgent) {
    console.log('🔄 Stage 8.5: Triggering multi-agent analysis (borderline case)...');
    try {
      const multiAgentResult = await analyzeWithMultiAgent(
        analysis,
        fileIdentification,
        entropyAnalysis,
        threatScore,
  ...
  const sandboxPrediction = await predictSandboxBehavior(
    analysis,
    threatScore,
    steganography,
    fileIdentification
