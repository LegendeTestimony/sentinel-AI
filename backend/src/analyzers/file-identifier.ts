...
import { detectMagicBytes, getFileExtension, type MagicDetectionResult } from './magic-bytes.js';
import { parseISOBMFF, type ISOBMFFResult } from './isobmff-parser.js';
...
export interface FileIdentificationResult {
  ...
  confidenceBreakdown: {
    magicBytes: number;
    containerParsing: number;
    extensionMatch: number;
  };

  // Detailed results from each layer
  magicByteMatches: MagicDetectionResult[];
  containerInfo?: ISOBMFFResult;

  // Mismatch detection (for security)
  claimedExtension: string;
  extensionMismatch: boolean;
  mismatchSeverity: 'none' | 'minor' | 'suspicious' | 'critical';
 ...
 */
export function identifyFile(filename: string, buffer: Buffer): FileIdentificationResult {
    ...
    if (topMatch.fileType === 'isobmff') {
      ...
      if (isobmff.valid && isobmff.fileType) {
        ...
        result.confidence = Math.min(
          (result.confidenceBreakdown.magicBytes * 0.3 + isobmff.confidence * 0.7),
          100
        );
      } else {
  ...
  if (extensionMatch) {
    ...
    result.mismatchSeverity = calculateMismatchSeverity(
      claimedExtension,
 ...
 */
function checkExtensionMatch(extension: string, detectedType: string): boolean {
  // Extension mapping
 ...
 */
function calculateMismatchSeverity(
  extension: string,
  detectedType: string,
  category: string
): 'none' | 'minor' | 'suspicious' | 'critical' {
 ...
 */
function generateHumanDescription(result: FileIdentificationResult): string {
