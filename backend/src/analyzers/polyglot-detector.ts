 ...
 */
export function detectPolyglot(buffer: Buffer): PolyglotAnalysis {
  const validFormats: string[] = [];

  // Check each format
  if (isValidPDF(buffer)) validFormats.push('PDF');
  if (isValidZIP(buffer)) validFormats.push('ZIP');
  ...
  if (isValidJavaScript(buffer)) validFormats.push('JavaScript');
  if (isValidPE(buffer)) validFormats.push('PE Executable');
  if (isValidScript(buffer)) validFormats.push('Shell Script');
 ...
 */
function assessPolyglotRisk(formats: string[]): {
  risk: 'low' | 'medium' | 'high' | 'critical';
  dangerousCombinations: string[];
} {
  const dangerousCombinations: string[] = [];
 ...
 */
function isValidHTML(buffer: Buffer): boolean {
  try {
 ...
 */
function isValidJavaScript(buffer: Buffer): boolean {
  try {
    const text = buffer.toString('utf-8', 0, Math.min(buffer.length, 1024));
    const jsPatterns = [
      /function\s+\w+\s*\(/,
 ...
 */
function isValidPE(buffer: Buffer): boolean {
 ...
 */
function isValidScript(buffer: Buffer): boolean {
  if (buffer.length < 3) return false;
