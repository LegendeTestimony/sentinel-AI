...
export interface SteganographyAnalysis {
  ...
  analysis: string;
  // Extracted hidden content for Gemini analysis
  extractedData?: {
    textMessages: string[];      // Human-readable text found
    rawDataSamples: string[];    // Hex samples of hidden binary data
    totalHiddenBytes: number;    // Total bytes of hidden data detected
    dataLocations: string[];     // Where hidden data was found
  };
}
...
export function detectSteganography(
  ...
  const techniques: SteganographyTechnique[] = [];
 ...
 */
function tryExtractText(buffer: Buffer): string | null {
  // Try UTF-8 first
 ...
 */
function listPNGChunks(buffer: Buffer): Array<{ type: string; length: number; offset: number }> {
  ...
  try {
    ...
    while (offset < buffer.length - 12) {
      ...
      if (length < 0 || length > 50000000) break;
 ...
 */
function extractLSBFromPNG(buffer: Buffer): string | null {
  try {
 ...
 */
function extractLSB_MSBFirst(pixelData: Buffer): string | null {
 ...
 */
function extractLSB_LSBFirst(pixelData: Buffer): string | null {
  const maxBytes = Math.min(pixelData.length, 20000);
  let message = '';
 ...
 */
function extractRGB_LSB(pixelData: Buffer): string | null {
 ...
 */
function extractLSB_SkipFilter(pixelData: Buffer): string | null {
 ...
 */
function extractSequentialRGB(pixelData: Buffer): string | null {
  ...
  for (let pixel = 0; pixel < maxPixels; pixel++) {
    ...
    if (baseIdx + 2 >= pixelData.length) break;
 ...
 */
function extractManyToolsStyle(pixelData: Buffer): string | null {
 ...
 */
function detectJPEGAppendedData(buffer: Buffer): { found: boolean; size: number; offset: number } {
 ...
 */
function detectPNGAppendedData(buffer: Buffer): { found: boolean; size: number; offset: number } {
 ...
 */
function performChiSquareJPEG(buffer: Buffer): { suspicious: boolean; confidence: number; evidence: string[] } {
 ...
 */
function detectSuspiciousPNGChunks(buffer: Buffer): { found: boolean; evidence: string[] } {
 ...
 */
function detectLargeHiddenData(buffer: Buffer, fileType: string): { found: boolean; evidence: string[] } {
  ...
  const highEntropyBlocks: number[] = [];
 ...
 */
function calculateBlockEntropy(block: Buffer): number {
 ...
 */
function generateSteganographyReport(
  techniques: SteganographyTechnique[],
  extractedData?: {
    textMessages: string[];
    rawDataSamples: string[];
