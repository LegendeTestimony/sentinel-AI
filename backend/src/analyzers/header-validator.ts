import type { HeaderValidation } from '../types/index.js';
 ...
 */
function getFileHeader(buffer: Buffer, bytes = 8): string {
 ...
 */
function detectFromMagicNumber(header: string): string {
...
export function validateHeader(
  ...
  if (extensionCount > 1) {
    const parts = filename.split('.');
    const hasExecutableExtension = ['exe', 'dll', 'bat', 'cmd', 'ps1', 'sh'].some(
      ext => parts.includes(ext)
    );
